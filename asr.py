    ...
    def __init__(self, config, width, height, num_classes):#20,80
        ...
        for i, filter_size in enumerate(self.config.filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                ...
                conv = tf.layers.conv1d(input_x, self.config.num_filters, filter_size)
        ...
        fc = tf.nn.relu(fc)'''
        # 分类器
        self.logits = tf.layers.dense(pooled_flat, num_classes, name='fc2')
...
def mfcc_batch_generator(batch_size=128, path = 'data/spoken_numbers_pcms/'):
  batch_features = []
  labels = []
    ...
    for epoch in range(config.num_epochs):
        ...
        for x_batch, y_batch in batch_train:
            ...
            if total_batch%config.print_per_batch == 0:
                ...
                valid_loss, valid_accuracy = session.run([cnn.loss, cnn.acc], feed_dict = {cnn.input_x: valid_features,cnn.input_y: valid_labels,cnn.keep_prob: config.dropout_keep_prob})
                print('Epoch'+str(total_batch))
                print ('train_loss:'+str(train_loss)+' train accuracy:'+str(train_accuracy) + '\tvalid_loss:' + str(valid_loss) + ' valid accuracy:' + str(valid_accuracy))
            if total_batch%config.save_tb_per_batch == 0:
                ...
                valid_s = session.run(merged_summary, feed_dict = {cnn.input_x: valid_features,cnn.input_y: valid_labels,cnn.keep_prob: config.dropout_keep_prob})
...
'''
1.数据归一化，对比未成初始化，速度明显快很多
2.减小卷积核大小，添加卷积层数，效果好很多 epoch 88 train_loss:0.13243791 train accuracy:1.0   valid_loss:0.49615747 valid accuracy:0.83958334
'''
